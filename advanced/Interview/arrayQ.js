...
function usageCount(ladder) {
    var count = 0;
    var targetNumber = 0; // The number you want to reach
    // Check if the ladder array is empty.
    if (ladder.length === 0) {
        return 0;
    }
    var found = false;
    var ladderCopy = __spreadArray([], ladder, true); // Create a copy of the original ladder array
    ...
    while (!found) {
        for (var i = 0; i < ladderCopy.length; i++) {
            ...
            console.log(value, 'val');
            value--;
            console.log(value, 'after minus');
            ladderCopy[i] = value; // Update the copy of the array with the decremented value.
        }
        count++;
        found = false;
    }
